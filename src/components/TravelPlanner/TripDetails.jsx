      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-gray-600">
              {trip.startDate} {t('trips.to', 'to')} {trip.endDate} ({calculateDuration(trip.startDate, trip.endDate)} {t('trips.days', 'days')})
            </p>
          </div>
        ...
        <div className="border-b border-gray-200 mb-6">
          <div className="flex flex-wrap -mb-px">
            ...
            >
              {t('tripDetails.overview', 'Overview')}
            </button>
            <button
              className={`mr-2 py-2 px-4 font-medium ${
                activeTab === 'itinerary' 
            ...
            >
              {t('tripDetails.itinerary', 'Itinerary')}
            </button>
            <button
              className={`mr-2 py-2 px-4 font-medium ${
                activeTab === 'tasks' 
            ...
            >
              {t('tripDetails.expenses', 'Expenses')}
            </button>
            <button
              className={`mr-2 py-2 px-4 font-medium ${
                activeTab === 'share' 
        ...
        {activeTab === 'overview' && (
          <div>
            {/* Trip Budget */}
            <div className="mb-8">
              <h3 className="text-xl font-semibold mb-4 text-gray-800">{t('tripDetails.budget', 'Budget')}</h3>
              <div className="bg-blue-50 p-4 rounded-lg">
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-1">
                    <h4 className="font-semibold mb-2">{t('budget.totalBudget', 'Total Budget')}</h4>
                    <p className="text-2xl font-bold text-blue-700">${trip.budget}</p>
                    {trip.budgetBreakdown && (
                      <div className="mt-3 space-y-1">
                        <p><span className="font-medium">{t('budget.accommodation', 'Accommodation')}:</span> ${trip.budgetBreakdown.accommodation}</p>
                        <p><span className="font-medium">{t('budget.food', 'Food')}:</span> ${trip.budgetBreakdown.food}</p>
                        <p><span className="font-medium">{t('budget.transportation', 'Transportation')}:</span> ${trip.budgetBreakdown.transportation}</p>
                        <p><span className="font-medium">{t('budget.activities', 'Activities')}:</span> ${trip.budgetBreakdown.activities}</p>
                        <p><span className="font-medium">{t('budget.other', 'Other')}:</span> ${trip.budgetBreakdown.other}</p>
                      </div>
                    )}
                  </div>
                  
                  <div className="flex-1">
                    <h4 className="font-semibold mb-2">{t('tripDetails.budgetVisualization', 'Budget Visualization')}</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div>
                <h3 className="text-xl font-semibold mb-4 text-gray-800">{t('tripDetails.transportation', 'Transportation')}</h3>
                    ...
                    <div className="space-y-3">
                      {trip.transports.map(t => (
                        <div key={t.id} className="bg-white p-3 rounded-lg shadow-sm">
                          ...
                          <div className="text-sm">{t('transport.from', 'From')}: {t.from}</div>
                          <div className="text-sm">{t('transport.to', 'To')}: {t.to}</div>
                          {t.price && <div className="text-sm">{t('transport.price', 'Price')}: ${t.price}</div>}
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div>
                <h3 className="text-xl font-semibold mb-4 text-gray-800">{t('tripDetails.tripTasks', 'Trip Tasks')}</h3>
                    ...
                    <div className="space-y-2">
                      {trip.tasks.slice(0, 5).map(task => (
                        <div key={task.id} className="bg-white p-3 rounded-lg shadow-sm flex items-center">
                          ...
                          <div className="flex-1">
                            ...
                            {task.date && <div className="text-xs text-gray-500">{t('tasks.dueDate', 'Due')}: {task.date}</div>}
                          </div>
                        </div>
                      ))}
                      ...
                      {trip.tasks.length > 5 && (
                        <div className="text-center mt-2">
                          ...
                          >
                            {t('tripDetails.viewAllTasks', 'View all')} {trip.tasks.length} {t('tripDetails.tasks', 'tasks')}
                          </button>
                        </div>
                      )}
                    </div>
            ...
            {/* Photos Gallery */}
            <div className="mb-8">
              <h3 className="text-xl font-semibold mb-4 text-gray-800">{t('tripDetails.photoGallery', 'Photo Gallery')}</h3>
            ...
            {/* Notes Section */}
            <div className="mb-8">
              <h3 className="text-xl font-semibold mb-4 text-gray-800">{t('tripDetails.tripNotes', 'Trip Notes')}</h3>
            ...
            {/* External Services */}
            <div>
              <h3 className="text-xl font-semibold mb-4 text-gray-800">{t('tripDetails.externalServices', 'External Services')}</h3>
              {trip.externalServices && trip.externalServices.length > 0 ? (
                ...
                <div className="bg-gray-50 p-4 rounded-lg text-center">
                  <p className="text-gray-500">{t('tripDetails.noExternalServices', 'No external services added to this trip.')}</p>
                </div>
              )}
            </div>
          </div>
            ...
            {(!trip.tasks || trip.tasks.length === 0) ? (
              ...
              <div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <span className="font-medium">
                        {trip.tasks.filter(task => task.completed).length} {t('tasks.of', 'of')} {trip.tasks.length} {t('tripDetails.tasksComplete', 'tasks complete')}
                      </span>
                    ...
                    <div key={category} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                      <div className="bg-gray-100 px-4 py-2 font-medium capitalize">
                        ...
                        <span className="ml-2 text-sm text-gray-600">
                          ({categoryTasks.filter(t => t.completed).length}/{categoryTasks.length} {t('tasks.completed', 'completed')})
                        </span>
                      </div>
                      <div>
                            ...
                            <div className="flex-1">
                              ...
                              <div className="mt-1 flex text-xs space-x-2">
                                ...
                                <span className={`px-2 py-0.5 rounded-full bg-gray-100 ${
                                  ...
                                  {t(`tasks.priorities.${task.priority}`, task.priority.charAt(0).toUpperCase() + task.priority.slice(1))} {t('tripDetails.priority', 'Priority')}
                                </span>
                              </div>
                            </div>
                            <div className="ml-2 flex items-center">
                              {task.completed ? (
                                <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">
                                  {t('tripDetails.completed', 'Completed')}
                                </span>
                              ) : (
                                <span className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded-full">
                                  {t('tripDetails.pending', 'Pending')}
                                </span>
                              )}
                            </div>
                          </div>
                ...
                <div className="mt-6 text-center">
                  ...
                  >
                    {t('tripDetails.manageTasks', 'Manage Tasks')}
                  </button>
                </div>
              </div>
            )}
        ...
        {activeTab === 'share' && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="font-semibold mb-3">{t('tripDetails.shareViaSocial', 'Share via Social Media')}</h4>
            ...
            {/* Collaborate Section */}
            <div className="mt-6">
              <h4 className="font-semibold mb-3">{t('tripDetails.collaborateWithCompanions', 'Collaborate with Travel Companions')}</h4>
              <div className="bg-white border border-gray-200 p-4 rounded-lg">
                <p className="text-gray-600 mb-3">
                  {t('tripDetails.inviteToContribute', 'Invite others to view and contribute to your trip planning.')}
                </p>
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    ...
                    <select className="p-2 border border-gray-300 rounded">
                      <option value="view">{t('tripDetails.canView', 'Can View')}</option>
                      <option value="edit">{t('tripDetails.canEdit', 'Can Edit')}</option>
                    </select>
                    <button className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                      {t('tripDetails.invite', 'Invite')}
                    </button>
                  </div>
                  
                  <div className="bg-blue-50 p-3 rounded-lg">
                    <h5 className="font-medium mb-2">{t('tripDetails.sharedWith', 'Shared With')}</h5>
                    {/* In a real app, this would show actual collaborators */}
                    <p className="text-gray-500 text-sm italic">{t('tripDetails.noOneYet', 'No one yet. Invite companions above.')}</p>
                  </div>
                </div>
              </div>
            </div>
